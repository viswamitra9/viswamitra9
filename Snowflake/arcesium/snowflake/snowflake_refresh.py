...
def update_current_running_refresh_status_on_desflow(status_message, files_to_attach=[]):
    ...
    if row is None:
        logger.error(
            'There are no DESFlow request details in table [dbainfra].[dbo].[refresh_desflow_ticket_details] in DBMONITOR')
        logger.error('Cannot update status to DESFlow. Returning')
        return False
    sub = "ArcTechOps#" + str(row.archelpnumber) + ": " + row.archelpsubject
    snowflakeutil.send_mail('dba-ops@arcesium.com', ['dba-requests@arcesium.com'], sub, status_message,
                            files_to_attach)
...
def check_refresh_possibility(dest_pod):
    ...
    query = "select count(1) as cnt from dbainfra.dbo.refresh_server_inventory " \
            "where lower(dest_pod) = '"+ str(dest_pod).lower()+"' and performrefresh=1"
    cur_sql_dest, conn_sql_dest = snowflakeutil.sql_connect()
    cur_sql_dest.execute(query)
...
def replicate_database_from_source(source_account, destination_account, destination_pod, dbname, source_pod):
    ...
    2. Refresh the cocoa_new database which will copy the database cocoa from production to uat
    3. create a clone of the cocoa_new with name cocoa_clone
...
def check_replication(source_account, source_pod, destination_account, destination_pod):
    ...
    try:
        ...
        if count == 0:
            logger.error("replication is not enabled "
                         "from source pod : {} to destination pod : {}".format(source_pod, destination_pod))
            return 1
        return 0
    except Exception as e:
        logger.error("Error occured while verifying replication between source pod : {} and destination pod : {}".format(source_pod, destination_pod))
        raise Exception("Error occured while verifying replication between source pod : {} and destination pod : {}".format(source_pod, destination_pod))
...
def enable_replication(source_account, source_pod, destination_account, destination_pod, dbname):
    ...
    try:
        ...
        logger.info(
            "enabling replication for database {} from source pod {} to destination pod {}".format(dbname, source_pod,
                                                                                                   destination_pod))
        ...
        logger.info("enabled replication for database {} from source pod {} to destination pod".format(dbname, source_pod,
                                                                                                       destination_pod))
        return 0
    except Exception as e:
        logger.error("Error occured while enabling replication for database {} between source pod : {} and destination pod : {}".format(
            dbname,source_pod, destination_pod))
        raise Exception("Error occured while enabling replication for database {} between source pod : {} and destination pod : {}".format(
            dbname,source_pod, destination_pod))
...
def backup_stages(destination_account, destination_pod, dbname):
    try:
        ...
        cursor.execute("select \"schema_name\"||'.'||\"name\" as ffromat_name from table(result_scan(last_query_id()))")
        result = cursor.fetchall()
        if len(result) > 0:
            for var in result:
                format_name = var[0]
                cursor.execute("insert into audit_archive.public.stage_backup select '{}',1, get_ddl('file_format','{}',TRUE)".format(dbname,format_name))
        ...
        logger.error("Error {} occured while taking backup of stages/pipes/tasks/streams from pod {} in database {}".format(str(e),destination_pod, dbname))
        raise Exception("Error {} occured while taking backup of stages/pipes/tasks/streams from pod {} in database {}".format(str(e),destination_pod, dbname))


def backup_users_roles_permissions(destination_account, destination_pod):
    """
...
def restore_stages_permissions(destination_account, destination_pod, dbname):
    """
    ...
    try:
        ...
        cursor.execute("select def from audit_archive.public.stage_backup order by ordr")
        result = cursor.fetchall()
        if len(result) > 0:
            for var in result:
                sql_statement = var[0]
        ...
        cursor.execute("show shares")
        cursor.execute("select \"name\" from table(result_scan(last_query_id()))"
                       " where \"kind\"='OUTBOUND' and \"database_name\"='{}_OLD'".format(str(dbname).upper()))
        result = cursor.fetchall()
        if len(result) > 0:
            for var in result:
                share_name = var[0]
        ...
        from audit_archive.public.dbgrants where granted_to='SHARE' and NAME like '{}_OLD%'
        ...
        logger.info("Granting permissions on database {} to shares".format(dbname))
        query = """
    ...
    try:
        ...
        if retcode == 1:
            ...
            status_message = "Refresh environment \n      Source: {} \n      Destination: {} \n Status: " \
                             ...
                             "Please check the attached log file for further details".format(source_pod, destination_pod)
            update_current_running_refresh_status_on_desflow(status_message=status_message,files_to_attach=[logfile])
            update_current_running_status(source_pod, destination_pod, statuscode=22,comments='Refresh is not scheduled for this pod')
            sys.exit(1)
        ...
        destination_cursor.execute("create or replace table audit_archive.public.stage_properties (dbname varchar,schemaname varchar, stagename varchar, "
                       "parent_property varchar, property varchar, property_type varchar, "
                       "property_value varchar, property_default varchar)")
        destination_cursor.execute("create or replace table audit_archive.public.stage_backup(dbname varchar,ordr int,def varchar)")
        """
        ...
        status_message = "Refresh environment\n      Source: {}\n      Destination: {}\n" \
                 ...
                 "Please check the attached log file for further details".format(source_pod, destination_pod)
        update_current_running_refresh_status_on_desflow(status_message=status_message,files_to_attach=[logfile])
        return_code = backup_users_roles_permissions(destination_account, destination_pod)
        if return_code != 0:
        ...
        """
        Snowflake replication work for for database, in a Snowflake account there are multiple databases.
        ...
        return_code = restore_stages_permissions(destination_account, destination_pod, dbname)
        if return_code != 0:
